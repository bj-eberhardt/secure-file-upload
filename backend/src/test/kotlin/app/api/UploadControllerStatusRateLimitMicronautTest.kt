...
class UploadControllerStatusRateLimitMicronautTest : TestPropertyProvider {
    @Inject
    @field:Client("/")
    lateinit var client: HttpClient

    private val storageDir: Path = TestStorageDirs.createTempStorageDir("secure-file-upload-statusrl-it")
    ...
    override fun getProperties(): MutableMap<String, String> = mutableMapOf(
        ...
        "secure-file-upload.download-requests-per-minute" to "1",
    ...
    @Test
    fun status_secondRequestIs429_withRetryAfter() {
        ...
        val first = client.toBlocking().exchange(HttpRequest.GET<Any>("/api/v1/uploads/$id/status"), UploadStatusResponse::class.java)
